/*
  ...
  https://github.com/superwills/NibbleAndAHalf -- Fast base64 encoding and
  decoding. version 1.0.1, Feb 1, 2022 812a
...
// You can clearly see here why base64 encoding is a really bloated
// representation of the original data: look how many entries are unused. Each
// ascii character can index any value between 0-255 in an array, but we're only
// using 64 of the available slots for meaningful values, leaving 192/256 values
// unused.
...
char *base64(const void *binaryData, int len, int *flen) {
    ...
    // PAD. Base64 is all about breaking the input into SEXTETS, or 6-bit
    // inputs. If you have 1 byte of input, that's 8 bits, not 6. So it won't
    // fit. We need the input to be a multiple of 6. So 8 bits would be padded
    ...
    int pad = ((lenMod3 & 1) << 1) +
              ((lenMod3 & 2) >> 1); // 2 gives 1 and 1 gives 2, but 0 gives 0.
...
// Talking in bits, the input already HAS to be a multiple of 8 (because you
// just can't have anything smaller than a byte saved to memory or disk on
// modern computers). To successfully convert the bitstream into groups of 6
// bits, we'll force the input bitstream to being a MULTIPLE OF 24, so that it
// will evenly divide by 6.
...
// Note that no mask needed since BYTE0 is unsigned, so 0's always come in from
// left (even though there is implicit int promotion on R&L sides prior to
// actual bitshift).

// the second sextet BBBBBB is part of the first byte and partly in the 2nd
// byte.
//   BYTE0       BYTE1
// AAAA AABB   BBBB CCCC
// The first part takes the lower 2 bits of the first byte and pushes them
// LEFT 4: (AAAA AABB becomes 00BB 0000), then bitwise ORs to it the top 4 bits
// of BYTE1, shifted RIGHT 4 (BBBB CCCC becomes 0000 BBBB).
#define SEXTET_B(byte0, byte1) (((0x3 & byte0) << 4) | (byte1 >> 4))

// 3rd sextet CCCCCC is lower nibble of 2nd byte and upper half nibble of 3rd
// byte.
//   BYTE1       BYTE2
// BBBB CCCC   CCDD DDDD
...
// in BYTE2. So we just shift BYTE2 right by 6 bits (CCDD DDDD becomes 0000
// 00CC).
...
#define SEXTET_D(byte2) (0x3f & byte2)
    ...
    int i = 0, byteNo; // result counter, and which byte we're on of the
                       // original source data.
    // I still need these variables after the loop
    for (byteNo = 0;
         byteNo <=
         len -
             3; // This loop is NOT entered for if there
                // are trailing bytes that are not a multiple of 3 bytes,
                // since we skip in 3's.
                // If there WAS padding, skip the last 3 octets and process
                // below. 0=>no, 1=>no, 2=>no, 3=>ONCE,4=>ONCE,5=>ONCE, 6=>2x..
    ...
    // The last 3 octets must be converted carefully as if len%3==1 or len%3==2
    // we must "pretend" there are additional bits at the end.
    if (pad == 1) {
        unsigned char BYTE0 = bin[byteNo];
        unsigned char BYTE1 = bin[byteNo + 1];
        ...
        // SEXTET3(BYTE1, 0), but to save some ops we just write what will
        // actually be needed here only.
    ...
    } else if (pad == 2) // len%3==1 (1,4,7,10)
    {
        ...
        //    - are being specified by the lowest 2 bits of the 1st octet. these
        //    should be 0.
        //     bin[0]       bin[1]      bin[2]
        //  +-----------+-----------+-----------+
        //  | 0000 0011   ~~~~ ~~~~   ~~~~ ~~~~ |
        ...
        base64String[i++] =
            b64[(0x3 & BYTE0)
                << 4]; // "padded" by 0's, these 2 bits are still HI ORDER BITS.
...
unsigned char *unbase64(const char *ascii, int len, int *flen) {
    ...
    const unsigned char *safeAsciiPtr =
        (const unsigned char *)ascii; // internally I use
    ...
    // This eliminates a possible bounds check on your array lookups into
    // unb64[]
    // (*(unsigned char*) having values between 0 and 255 means it will always
    // be inside the bounds of the 256 element array).
...
int base64integrity(const char *ascii, int len) {
    ...
    // represents only a 6-bit value (a sextet). That leaves 2 bits wasted per 8
    // bits used. More importantly, for the sextet stream you're getting here
    // (inside an octet stream) to be VALID, THE LENGTH HAS TO BE A MULTIPLE
    // OF 4. You can see in the base64 function above, the algorithm always
    // writes into the final base64 string in groups of __4__.
    ...
    // Here, the empty string will be valid base64 because it represents empty
    // data.
    if (len % 4)
        return 0;
    ...
    for (i = 0; i < len - 2; i++) {
        if (!isbase64ValidChr(ascii[i])) {
            printf("ERROR in base64integrity at chr %d [%c]. String is NOT "
                   "valid base64.\n",
                   i, ascii[i]);
            return 0;
        }
    ...
    if (ascii[i] == '=') {
        ...
        if (ascii[i + 1] != '=') {
            printf("ERROR in base64integrity at chr %d.\n"
                   "If the 2nd last chr is '=' then the last chr must be '=' "
                   "too.\n "
    ...
    } else if (!isbase64ValidChr(ascii[i])) // not = or valid base64
    {
        ...
        printf("ERROR in base64integrity at chr %d (2nd last chr). String is "
               "NOT valid base64.\n",
               i);
        return 0;
    }
    ...
    if (ascii[i] != '=' && !isbase64ValidChr(ascii[i])) {
        printf("ERROR in base64integrity at chr %d (last chr). String is NOT "
               "valid base64.\n",
               i);
        return 0;
    }
